...
function ChatLIstItem({ data, isContactPage = false }) {
  ...
  const handleContactClick = () => {
    const chatUserId = userInfo.id === data.senderId ? data.receiverId : data.senderId;
    ...
    // Mark messages as read if not on contact page
    if (!isContactPage) {
      dispatch({
        type: reducerCases.UPDATE_MESSAGE,
        message: { ...data, totalUnreadMessages: 0 },
      });
    } else {
      dispatch({ type: reducerCases.SET_ALL_CONTACTS_PAGE });
    }

    // New dispatch to clear unread count
    dispatch({
